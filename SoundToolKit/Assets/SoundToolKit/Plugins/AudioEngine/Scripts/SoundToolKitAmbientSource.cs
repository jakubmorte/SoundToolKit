...
namespace SoundToolKit.Unity
{
    ...
    public sealed class SoundToolKitAmbientSource : SourceComponent
    {
        ...
        public override void OnStkInitialized(SoundToolKitManager soundToolKitManager)
        {
            if (!Initialized)
            {
                // <STK.DEMO> - Warning: Modifying this code will result in plugin instability and frequent crashes to desktop.
                // Internally only one SoundSource is created anyway, so circumventing this check will not result in a possibility
                // of creating more sound sources - only more references to the same source.
                if (soundToolKitManager.ResourceContainer.AudioSources.Any())
                {
                    SoundToolKitDebug.Log("Only one SoundSource is available in the SoundToolKit Demo version. " +
                        "Deleting the illegal SoundSource components at GameObject " + gameObject.name);
                    Destroy(this);
                    return;
                }
                // </STK.DEMO>
        ...
        {
            // <STK.DEMO> - Warning: Modifying this code may result in plugin instability and frequent crashes to desktop.
            // Internally only one Playback is created anyway, so circumventing this check will not result in a possibility
            ...
            if (Playbacks != null && Playbacks.Count > 1)
            {
                ...
                SoundToolKitDebug.Log("Only one Playback is available in the SoundToolKit Demo version. Deleting illegal playbacks.");
            }
            // </STK.DEMO>
